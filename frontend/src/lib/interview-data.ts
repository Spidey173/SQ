...
export interface ProblemInterviewDataset {
  id: string;
  title: string;
  levelNumber: number;
...
export const ALL_50_INTERVIEW_DATA: Record<number, ProblemInterviewDataset> = {
  "1": {
    ...
    "levelNumber": 1,
    "problemId": 1,
    "problemTitle": "Select all columns from a table",
    "difficulty": "Easy",
    "companyTags": [
      "Amazon",
      "Google",
      ...
      "steps": [
        {
          ...
          "explanation": "The SQLite engine parses the query syntax for 'Select all columns from a table' and validates schema column names."
        },
        {
          "step": 2,
          "lineNumber": 1,
    ...
    "qas": [
      {
        ...
        "whatInterviewerChecks": "Understanding of basic SQL query commands.",
        "bestReplyScript": "It retrieves data from a database table.",
        "commonMistakesToAvoid": "Thinking SELECT modifies data in the database.",
        "keyPoints": ["Data Retrieval", "Read-Only Operation"],
        "codeSnippet": "SELECT * FROM table_name;"
      },
      {
        "id": "q-1-2",
        "category": "💡 Interview Notes",
        "question": "What does * mean?",
        "whatInterviewerChecks": "Understanding of the wildcard operator in relational projections.",
        "bestReplyScript": "It represents all columns in the table.",
        "commonMistakesToAvoid": "Assuming * filters rows.",
        "keyPoints": ["Wildcard Column Selector", "All Columns"],
        "codeSnippet": "SELECT *"
      },
      {
        "id": "q-1-3",
        "category": "💡 Interview Notes",
        "question": "Does SELECT * return all rows?",
        "whatInterviewerChecks": "Understanding row filtering and clause interaction.",
        "bestReplyScript": "Yes. If there is no WHERE clause, SQL returns every row.",
        "commonMistakesToAvoid": "Confusing column wildcard with row filtering.",
        "keyPoints": ["All Rows Returned", "No Filter Condition"],
        "codeSnippet": "SELECT * FROM table_name;"
      },
      {
        "id": "q-1-4",
        "category": "💡 Interview Notes",
        "question": "Can SELECT * be used on any table?",
        "whatInterviewerChecks": "Table access permissions and database object existence.",
        "bestReplyScript": "Yes, as long as the table exists and you have permission to access it.",
        "commonMistakesToAvoid": "Querying non-existent tables or ignoring table-level permissions.",
        "keyPoints": ["Table Existence", "Access Permissions"],
        "codeSnippet": "SELECT * FROM table_name;"
      }
    ],
    "questions": [
    ...
    "mistakes": [
      {
        "id": "m-1-1",
        "title": "1. Forgetting the FROM clause",
        "description": "SQL doesn't know which table to read data from.",
        "badSnippet": "SELECT *;",
        "failingInput": "SELECT *;",
        "consequence": "❌ Error: SQL doesn't know which table to read.",
        "howToFix": "Always specify the target table using FROM table_name; (e.g., SELECT * FROM table_name;)",
        "mistake": "Missing FROM clause",
        "whyItHappens": "Forgetting that SELECT only specifies columns, while FROM specifies the table."
      },
      {
        "id": "m-1-2",
        "title": "2. Incorrect table name",
        "description": "If your schema table is table_name, using an incorrect name will fail.",
        "badSnippet": "SELECT *\nFROM wrong_table_name;",
        "failingInput": "FROM wrong_table_name;",
        "consequence": "❌ Error: no such table: wrong_table_name",
        "howToFix": "Check table schema spelling and use the exact table name defined in schema.sql.",
        "mistake": "Incorrect table name spelling",
        "whyItHappens": "Guessing table names or querying tables that haven't been created."
      },
      {
        "id": "m-1-3",
        "title": "3. Misspelling SQL keywords",
        "description": "SELET is not a valid SQL keyword.",
        "badSnippet": "SELET *\nFROM table_name;",
        "failingInput": "SELET *",
        "consequence": "❌ Error: Syntax error near SELET",
        "howToFix": "Ensure keywords are spelled correctly: SELECT * FROM table_name;",
        "mistake": "Typo in SQL keyword",
        "whyItHappens": "Typing fast without syntax highlighting check."
      }
    ]
  },
  "2": {
    ...
    "levelNumber": 2,
    "problemId": 2,
    "problemTitle": "Select Specific Columns",
    "difficulty": "Easy",
    "companyTags": [
      "Amazon",
      "Google",
      ...
      "steps": [
        {
          ...
          "explanation": "The database engine parses the query syntax for 'Select Specific Columns' and validates column names in table employees."
        },
        {
          "step": 2,
          "lineNumber": 1,
          "vars": {
            "TableScan": "Scanning employees",
            "RowsMatched": "All tuples"
          },
          "explanation": "The storage engine scans data pages from the employees table."
        },
        {
          "step": 3,
          "lineNumber": 1,
          "vars": {
            "Projection": "Output Column Buffer",
            "Columns": "employee_id, first_name, last_name, job_title, salary"
          },
          "explanation": "The projection engine filters out unrequested attributes (like email, city) and outputs only specified columns."
        }
      ]
    },
    "qas": [
      {
        ...
        "whatInterviewerChecks": "Understanding performance impact and production query best practices.",
        "bestReplyScript": "SELECT * retrieves all columns (useful for debugging/learning), while selecting specific columns retrieves only required attributes, making it faster, more memory-efficient, and preferred in production.",
        "commonMistakesToAvoid": "Using SELECT * in production API queries.",
        "keyPoints": ["Performance", "Memory Overhead", "Production Best Practice"],
        "codeSnippet": "SELECT employee_id, first_name, last_name, job_title, salary FROM employees;"
      },
      {
        "id": "q-2-2",
        "category": "💡 Interview Notes",
        "question": "Does selecting fewer columns improve performance?",
        "whatInterviewerChecks": "Understanding of I/O, memory usage, and network payload reduction.",
        "bestReplyScript": "Yes. Selecting fewer columns reduces the amount of data read from disk, lowers memory usage in database buffers, and decreases network transfer payload.",
        "commonMistakesToAvoid": "Thinking column selection has no performance impact.",
        "keyPoints": ["Reduced I/O", "Lower Memory Footprint", "Faster Network Payload"],
        "codeSnippet": "SELECT employee_id, first_name FROM employees;"
      },
      {
        "id": "q-2-3",
        "category": "💡 Interview Notes",
        "question": "Can columns be written in any order?",
        "whatInterviewerChecks": "Understanding projection ordering vs table schema ordering.",
        "bestReplyScript": "Yes. Columns can be listed in any order (e.g., SELECT salary, first_name, employee_id FROM employees;). The output result set will display columns in the exact order written in the query.",
        "commonMistakesToAvoid": "Assuming columns must match schema definition order.",
        "keyPoints": ["Projection Ordering", "Custom Result Headers"],
        "codeSnippet": "SELECT salary, first_name, employee_id FROM employees;"
      }
    ],
    "questions": [
      {
    ...
    "mistakes": [
      {
        "id": "m-2-1",
        "title": "1. Forgetting commas",
        "description": "Columns in SELECT list must be separated by commas.",
        "badSnippet": "SELECT employee_id first_name salary FROM employees;",
        "failingInput": "SELECT employee_id first_name salary FROM employees;",
        "consequence": "❌ Syntax Error: Unexpected identifier first_name.",
        "howToFix": "Separate every column name with a comma: SELECT employee_id, first_name, salary FROM employees;",
        "mistake": "Missing comma between column names",
        "whyItHappens": "Typing column names rapidly without commas."
      },
      {
        "id": "m-2-2",
        "title": "2. Misspelling a column name",
        "description": "If a column does not exist in the table schema, SQL returns an error.",
        "badSnippet": "SELECT employe_id FROM employees;",
        "failingInput": "SELECT employe_id FROM employees;",
        "consequence": "❌ Error: no such column: employe_id",
        "howToFix": "Check schema column names and correct spelling to employee_id.",
        "mistake": "Column name spelling mistake",
        "whyItHappens": "Typo in attribute name."
      },
      {
        "id": "m-2-3",
        "title": "3. Using the wrong table name",
        "description": "Querying a table name that differs from schema definition.",
        "badSnippet": "SELECT employee_id FROM employee;",
        "failingInput": "FROM employee",
        "consequence": "❌ Error: no such table: employee",
        "howToFix": "Ensure table name matches schema exactly: FROM employees;",
        "mistake": "Singular/plural table name error",
        "whyItHappens": "Using singular 'employee' instead of plural 'employees'."
      }
    ]
  },
  "3": {
    ...
    "levelNumber": 3,
    "problemId": 3,
    "problemTitle": "Filter Rows Using WHERE",
    "difficulty": "Easy",
    "companyTags": [
      "Amazon",
      "Microsoft",
    ...
    "tracing": {
      "code": "SELECT * FROM employees WHERE department_id = 101;",
      "steps": [
        {
          "step": 1,
          "lineNumber": 1,
          ...
          "explanation": "SQL locates the target table 'employees' to scan data pages."
        },
        {
          "step": 2,
          "lineNumber": 2,
          "vars": {
            ...
            "Action": "Evaluate condition for each row"
          },
          "explanation": "SQL evaluates predicate condition 'department_id = 101' for each row. Keeps matching rows (TRUE) and ignores non-matching rows (FALSE)."
        },
        {
          "step": 3,
          "lineNumber": 3,
          ...
          "explanation": "SQL returns all columns (*) for the filtered set of rows."
        }
      ]
    },
    "qas": [
      {
        ...
        "whatInterviewerChecks": "Understanding row filtering and query predicate evaluation.",
        "bestReplyScript": "The WHERE clause filters rows before SQL returns the result. Only rows where the condition evaluates to TRUE are included in the output.",
        "commonMistakesToAvoid": "Confusing WHERE (row filter) with SELECT (column selector).",
        "keyPoints": ["Row Filtering", "Predicate Condition", "Evaluates Before Projection"],
        "codeSnippet": "SELECT * FROM employees WHERE department_id = 101;"
      },
      {
        "id": "q-3-2",
        "category": "💡 Interview Notes",
        "question": "Does WHERE filter rows or columns?",
        "whatInterviewerChecks": "Core relational algebra concepts (selection vs projection).",
        "bestReplyScript": "WHERE filters rows. SELECT controls columns, while WHERE controls rows.",
        "commonMistakesToAvoid": "Thinking WHERE controls which columns appear.",
        "keyPoints": ["WHERE = Rows Filter", "SELECT = Columns Selector"],
        "codeSnippet": "WHERE department_id = 101"
      },
      {
        "id": "q-3-3",
        "category": "💡 Interview Notes",
        "question": "Can we use multiple conditions in WHERE?",
        "whatInterviewerChecks": "Boolean logic (AND / OR) usage in queries.",
        "bestReplyScript": "Yes. Multiple conditions can be combined using logical operators like AND or OR (e.g., WHERE department_id = 101 AND salary > 50000).",
        "commonMistakesToAvoid": "Mixing up AND vs OR logic operator precedence.",
        "keyPoints": ["Logical Operators", "AND / OR Conditions"],
        "codeSnippet": "WHERE department_id = 101 AND salary > 50000;"
      },
      {
        "id": "q-3-4",
        "category": "💡 Interview Notes",
        "question": "What happens if no row matches the WHERE condition?",
        "whatInterviewerChecks": "Result set behavior on zero-match predicates.",
        "bestReplyScript": "SQL returns an empty result set (0 rows). No database error is thrown.",
        "commonMistakesToAvoid": "Expecting a syntax or runtime error when 0 rows match.",
        "keyPoints": ["Empty Result Set", "No Error Thrown"],
        "codeSnippet": "WHERE salary > 9999999;"
      },
      {
        "id": "q-3-5",
        "category": "💡 Interview Notes",
        "question": "Which clause executes first: SELECT or WHERE?",
        "whatInterviewerChecks": "Understanding of logical query execution sequence (FROM ➔ WHERE ➔ SELECT).",
        "bestReplyScript": "WHERE executes before SELECT. The logical order is FROM ➔ WHERE ➔ SELECT. SQL first locates the table, filters matching rows, and then projects the requested columns.",
        "commonMistakesToAvoid": "Assuming SELECT executes first because it is written first.",
        "keyPoints": ["Logical Query Processing", "FROM ➔ WHERE ➔ SELECT"],
        "codeSnippet": "FROM ──► WHERE ──► SELECT"
      }
    ],
    "questions": [
      {
    ...
    "mistakes": [
      {
        "id": "m-3-1",
        "title": "1. Forgetting the WHERE keyword",
        "description": "Placing a condition directly after FROM without WHERE.",
        "badSnippet": "SELECT * FROM employees department_id = 101;",
        "failingInput": "SELECT * FROM employees department_id = 101;",
        "consequence": "❌ Syntax Error: SQL doesn't understand the condition.",
        "howToFix": "Add the WHERE keyword: SELECT * FROM employees WHERE department_id = 101;",
        "mistake": "Missing WHERE keyword",
        "whyItHappens": "Forgetting that conditions require explicit WHERE clause."
      },
      {
        "id": "m-3-2",
        "title": "2. Using == instead of =",
        "description": "Using programming equality == instead of SQL equality operator =.",
        "badSnippet": "SELECT * FROM employees WHERE salary == 50000;",
        "failingInput": "WHERE salary == 50000;",
        "consequence": "❌ Syntax Error in ANSI SQL.",
        "howToFix": "Use single = operator in SQL: WHERE salary = 50000;",
        "mistake": "Using == equality operator",
        "whyItHappens": "Habit from Python, Java, or C++ programming."
      },
      {
        "id": "m-3-3",
        "title": "3. Forgetting Quotes for Strings",
        "description": "Passing string literals without quotes.",
        "badSnippet": "SELECT * FROM employees WHERE city = Bangalore;",
        "failingInput": "WHERE city = Bangalore;",
        "consequence": "❌ Error: SQL thinks Bangalore is a column name.",
        "howToFix": "Wrap string values in single quotes: WHERE city = 'Bangalore';",
        "mistake": "Unquoted string literal",
        "whyItHappens": "Confusing string literal values with table column names."
      },
      {
        "id": "m-3-4",
        "title": "4. Wrong Column Name",
        "description": "Filtering on a column that does not exist in schema.",
        "badSnippet": "SELECT * FROM employees WHERE salaries > 50000;",
        "failingInput": "WHERE salaries > 50000;",
        "consequence": "❌ Error: Unknown column salaries.",
        "howToFix": "Verify schema column name: WHERE salary > 50000;",
        "mistake": "Misspelled column name",
        "whyItHappens": "Pluralizing column name."
      },
      {
        "id": "m-3-5",
        "title": "5. Using = with NULL",
        "description": "Attempting to check NULL equality with = instead of IS NULL.",
        "badSnippet": "WHERE manager_id = NULL;",
        "failingInput": "WHERE manager_id = NULL;",
        "consequence": "❌ Evaluates to UNKNOWN (returns 0 rows).",
        "howToFix": "Use IS NULL operator: WHERE manager_id IS NULL;",
        "mistake": "Equality comparison with NULL",
        "whyItHappens": "Not knowing NULL in SQL requires IS NULL operator."
      }
    ]
  },
  "4": {
    ...
    "levelNumber": 4,
    "problemId": 4,
    "problemTitle": "Use Multiple Conditions with AND",
    "difficulty": "Easy",
    "companyTags": [
      "Amazon",
      "Microsoft",
      ...
      "steps": [
        {
          ...
          "explanation": "SQL locates the 'employees' table to begin row inspection."
        },
        {
          "step": 2,
          "lineNumber": 3,
          "vars": {
            "Step": "WHERE department_id = 101",
            "Action": "Evaluate Checkpoint 1"
          },
          "explanation": "SQL inspects each row's department_id column. If department_id = 101 evaluates to TRUE, it proceeds to Checkpoint 2. If FALSE, the row is discarded."
        },
        {
          "step": 3,
          "lineNumber": 4,
          "vars": {
            "Step": "AND salary > 60000",
            "Action": "Evaluate Checkpoint 2"
          },
          "explanation": "SQL evaluates salary > 60000 for rows that passed Checkpoint 1. If TRUE, the row is kept; if FALSE, the row is discarded."
        },
        {
          "step": 4,
          "lineNumber": 1,
          ...
          "explanation": "SQL returns all columns (*) for rows where BOTH department_id = 101 AND salary > 60000 evaluated to TRUE."
        }
      ]
    },
    "qas": [
      {
        ...
        "whatInterviewerChecks": "Understanding conjunctive logical evaluation in database queries.",
        "bestReplyScript": "The AND operator combines multiple filtering criteria in a WHERE clause. Every condition connected by AND must evaluate to TRUE for a row to be included in the output result set.",
        "commonMistakesToAvoid": "Assuming only one condition needs to pass.",
        "keyPoints": ["Conjunctive Logic", "All Conditions Must Be True", "Row Filter"],
        "codeSnippet": "WHERE department_id = 101 AND salary > 60000;"
      },
      {
        "id": "q-4-2",
        "category": "💡 Interview Notes",
        "question": "What happens if one condition connected by AND is FALSE?",
        "whatInterviewerChecks": "Boolean truth table knowledge for AND logic.",
        "bestReplyScript": "If even one condition connected by AND evaluates to FALSE (or NULL), the entire WHERE expression evaluates to FALSE/UNKNOWN and the row is discarded.",
        "commonMistakesToAvoid": "Expecting partial condition matches to be returned.",
        "keyPoints": ["Strict Discard", "Truth Table", "Boolean Short-Circuit"],
        "codeSnippet": "TRUE AND FALSE ➔ Discard Row"
      },
      {
        "id": "q-4-3",
        "category": "💡 Interview Notes",
        "question": "Which operator returns fewer rows: AND or OR?",
        "whatInterviewerChecks": "Understanding query selectivity and restriction levels.",
        "bestReplyScript": "AND usually returns fewer rows because it is more restrictive—every condition must be TRUE. OR returns more rows because satisfying any single condition is sufficient.",
        "commonMistakesToAvoid": "Saying OR returns fewer rows.",
        "keyPoints": ["AND = Narrower/Fewer Rows", "OR = Broader/More Rows"],
        "codeSnippet": "AND = Restrictive | OR = Inclusive"
      },
      {
        "id": "q-4-4",
        "category": "💡 Interview Notes",
        "question": "Can we combine 3 or more AND conditions in a single query?",
        "whatInterviewerChecks": "Scalability of WHERE clause logical predicates.",
        "bestReplyScript": "Yes. You can chain as many AND conditions as required (e.g., WHERE department_id = 101 AND salary > 60000 AND city = 'Bangalore'). Every single condition must be TRUE for a row to match.",
        "commonMistakesToAvoid": "Limiting AND to only 2 conditions.",
        "keyPoints": ["Multiple Predicates", "Chained Logical AND"],
        "codeSnippet": "WHERE department_id = 101 AND salary > 60000 AND city = 'Bangalore';"
      },
      {
        "id": "q-4-5",
        "category": "💡 Interview Notes",
        "question": "Why do we use parentheses when mixing AND and OR?",
        "whatInterviewerChecks": "Operator precedence rules (AND has higher precedence than OR).",
        "bestReplyScript": "Because AND has higher precedence than OR in SQL logic evaluation. We use parentheses to explicitly define the intended evaluation order and avoid unintended logic bugs.",
        "commonMistakesToAvoid": "Omitting parentheses when combining AND and OR.",
        "keyPoints": ["Operator Precedence", "Parentheses Grouping", "Logic Accuracy"],
        "codeSnippet": "WHERE department_id = 101 AND (salary > 60000 OR city = 'Bangalore');"
      }
    ],
    "questions": [
      {
    ...
    "mistakes": [
      {
        "id": "m-4-1",
        "title": "1. Forgetting WHERE when using AND",
        "description": "Attempting to use AND without an initial WHERE clause keyword.",
        "badSnippet": "SELECT * FROM employees AND salary > 60000;",
        "failingInput": "SELECT * FROM employees AND salary > 60000;",
        "consequence": "❌ Syntax Error: AND cannot be used without a WHERE clause.",
        "howToFix": "Include the WHERE clause keyword: SELECT * FROM employees WHERE department_id = 101 AND salary > 60000;",
        "mistake": "Missing WHERE clause keyword",
        "whyItHappens": "Forgetting that AND connects WHERE conditions."
      },
      {
        "id": "m-4-2",
        "title": "2. Using comma instead of AND",
        "description": "Separating conditions in WHERE clause using commas.",
        "badSnippet": "SELECT * FROM employees WHERE department_id = 101, salary > 60000;",
        "failingInput": "WHERE department_id = 101, salary > 60000;",
        "consequence": "❌ Syntax Error: Conditions cannot be separated using commas.",
        "howToFix": "Use the AND keyword: WHERE department_id = 101 AND salary > 60000;",
        "mistake": "Using commas between WHERE conditions",
        "whyItHappens": "Confusing SELECT column lists with WHERE conditions."
      },
      {
        "id": "m-4-3",
        "title": "3. Misspelling the AND keyword",
        "description": "Typing AAD or ANDD instead of AND.",
        "badSnippet": "WHERE department_id = 101 AAD salary > 60000;",
        "failingInput": "WHERE department_id = 101 AAD salary > 60000;",
        "consequence": "❌ Syntax Error: Invalid keyword near AAD.",
        "howToFix": "Correct spelling to AND.",
        "mistake": "Typo in AND keyword",
        "whyItHappens": "Fast typing typos."
      },
      {
        "id": "m-4-4",
        "title": "4. Unquoted string literals with AND",
        "description": "Omitting single quotes around string values in AND queries.",
        "badSnippet": "WHERE city = Bangalore AND salary > 50000;",
        "failingInput": "WHERE city = Bangalore AND salary > 50000;",
        "consequence": "❌ Error: SQL treats Bangalore as a column name.",
        "howToFix": "Wrap string literal in single quotes: WHERE city = 'Bangalore' AND salary > 50000;",
        "mistake": "Unquoted string literal",
        "whyItHappens": "Treating string values as identifiers."
      }
    ]
  },
  "5": {
  ...
  "51": {
    ...
    "tracing": {
      ...
      "steps": [
        ...
        },
        {
          ...
          "explanation": "Rounds averages to 2 decimal places (HR: 55000.00, IT: 80000.00, Finance: 105000.00)."
        },
        {
          "step": 5,
          "lineNumber": 1,
        ...
        },
        {
          ...
          "lineNumber": 5,
          "vars": {
            "Phase": "ORDER BY average_salary ASC",
            "Action": "Sort ascending"
          },
          "explanation": "Ranks departments ascending: 1. HR (55000.00), 2. IT (80000.00), 3. Finance (105000.00)."
        },
        {
          "step": 7,
          "lineNumber": 6,
          "vars": {
            "Phase": "LIMIT 1",
            "Action": "Isolate bottom row"
          },
          "explanation": "Extracts the top row of the ascending list (HR, 55000.00) and discards subsequent rows."
        }
      ]
    },
    "qas": [
      {
        "id": "q-51-1",
        "category": "💡 Interview Notes",
        "question": "Why is ORDER BY ASC used instead of DESC?",
        "whatInterviewerChecks": "Sort direction impact on limit queries.",
        "bestReplyScript": "ORDER BY average_salary ASC ranks departments from lowest to highest average salary. Combining ASC with LIMIT 1 ensures that the single lowest-paying department is returned.",
        "commonMistakesToAvoid": "Writing ORDER BY DESC when requested for the lowest salary department.",
        "keyPoints": ["ASC sorts from smallest to largest", "Places lowest aggregate value at row #1", "Essential for bottom-N questions"],
        "codeSnippet": "ORDER BY average_salary ASC LIMIT 1"
      },
      {
        "id": "q-51-2",
        "category": "💡 Interview Notes",
        "question": "Why is MIN(salary) incorrect for this problem?",
        "whatInterviewerChecks": "Row-level aggregate vs group average distinction.",
        "bestReplyScript": "MIN(salary) retrieves the lowest single employee salary across the company, whereas the problem asks for the department with the lowest average salary. Evaluating average salary requires AVG(salary) grouped by department.",
        "commonMistakesToAvoid": "Using MIN(salary) to solve department average queries.",
        "keyPoints": ["MIN(salary) yields individual minimum", "AVG(salary) yields group central tendency", "Must use GROUP BY department_name with AVG()"],
        "codeSnippet": "AVG(salary)"
      },
      {
        "id": "q-51-3",
        "category": "💡 Interview Notes",
        "question": "What is the key difference between SQL-015 and SQL-016?",
        "whatInterviewerChecks": "Comparative analysis of aggregate ranking queries.",
        "bestReplyScript": "SQL-015 finds the highest average salary department using ORDER BY average_salary DESC, whereas SQL-016 finds the lowest average salary department using ORDER BY average_salary ASC. The grouping and aggregation logic are identical.",
        "commonMistakesToAvoid": "Changing the GROUP BY or SELECT logic instead of just the sort direction.",
        "keyPoints": ["SQL-015 uses DESC (highest)", "SQL-016 uses ASC (lowest)", "Identical aggregation pipeline"],
        "codeSnippet": "ORDER BY average_salary ASC"
      },
      {
        "id": "q-51-4",
        "category": "💡 Interview Notes",
        "question": "What is the ANSI SQL standard equivalent of LIMIT 1?",
        "whatInterviewerChecks": "Cross-database SQL standard knowledge.",
        "bestReplyScript": "The ANSI SQL standard syntax is FETCH FIRST 1 ROW ONLY, which works natively across Oracle 12c+, PostgreSQL, and DB2.",
        "commonMistakesToAvoid": "Relying strictly on MySQL LIMIT during ANSI compliance interviews.",
        "keyPoints": ["FETCH FIRST 1 ROW ONLY is ANSI standard", "Supported in Oracle 12c+, Postgres, DB2", "Ideal candidate answer for portability"],
        "codeSnippet": "FETCH FIRST 1 ROW ONLY"
      },
      {
        "id": "q-51-5",
        ...
        "question": "What is the logical execution order of this query?",
        "whatInterviewerChecks": "Query lifecycle pipeline mastery.",
        "bestReplyScript": "1. FROM employees -> 2. GROUP BY department_name -> 3. Calculate AVG(salary) -> 4. Evaluate ROUND(..., 2) -> 5. SELECT department_name, average_salary -> 6. ORDER BY average_salary ASC -> 7. LIMIT 1.",
        "commonMistakesToAvoid": "Thinking SELECT or LIMIT runs before ORDER BY.",
        "keyPoints": ["FROM -> GROUP BY -> AVG -> ROUND -> SELECT -> ORDER BY -> LIMIT"],
        "codeSnippet": "FROM -> GROUP BY -> AVG -> ROUND -> SELECT -> ORDER BY -> LIMIT"
      }
    ],
    ...
    "mistakes": [
      {
        "id": "m-51-1",
        "title": "1. Using DESC Instead of ASC",
        "description": "Writing ORDER BY average_salary DESC.",
        "badSnippet": "ORDER BY average_salary DESC LIMIT 1;",
        "failingInput": "Lowest salary department prompt",
        "consequence": "Returns the highest-paying department instead of the lowest-paying one.",
        "howToFix": "Use ORDER BY average_salary ASC (or ORDER BY average_salary).",
        "mistake": "Wrong sort direction",
        "whyItHappens": "Habitually copying ORDER BY DESC from top-N queries."
      },
      {
        "id": "m-51-2",
        "title": "2. Using MIN(salary) Instead of AVG(salary)",
        "description": "Writing MIN(salary) instead of AVG(salary).",
        "badSnippet": "SELECT department_name, MIN(salary) FROM employees GROUP BY department_name;",
        "failingInput": "Department with single low-paid worker and high mean salaries",
        "consequence": "Finds lowest individual salary rather than department with lowest average salary.",
        "howToFix": "Use AVG(salary).",
        "mistake": "Using wrong aggregate function",
        "whyItHappens": "Confusing lowest individual worker wage with lowest departmental average."
      },
      {
        "id": "m-51-3",
        "title": "3. Forgetting GROUP BY Clause",
        "description": "Calling AVG(salary) alongside department_name without GROUP BY.",
        "badSnippet": "SELECT department_name, ROUND(AVG(salary), 2) FROM employees ORDER BY 2 ASC LIMIT 1;",
        "failingInput": "Multi-department organization",
        "consequence": "Collapses the entire company into a single row with overall company average.",
        "howToFix": "Add GROUP BY department_name.",
        "mistake": "Missing GROUP BY clause",
        "whyItHappens": "Assuming aggregation automatically groups per distinct department."
      },
      {
        "id": "m-51-4",
        "title": "4. Forgetting LIMIT 1",
        "description": "Omitting LIMIT 1 from the query.",
        "badSnippet": "SELECT department_name, ROUND(AVG(salary), 2) AS average_salary FROM employees GROUP BY department_name ORDER BY average_salary ASC;",
        "failingInput": "Single department output requirement",
        "consequence": "Returns every department in ascending order instead of only the lowest.",
        "howToFix": "Append LIMIT 1 to isolate the bottom record.",
        "mistake": "Missing LIMIT clause",
        "whyItHappens": "Forgetting that 'find the department' requests a single record."
      },
      {
        "id": "m-51-5",
        "title": "5. Forgetting ROUND(..., 2)",
        "description": "Omitting ROUND and leaving raw floating point numbers.",
        "badSnippet": "SELECT department_name, AVG(salary) AS average_salary FROM employees GROUP BY department_name ORDER BY average_salary ASC LIMIT 1;",
        "failingInput": "Formatted decimal requirement",
        "consequence": "Produces raw recurring floating point numbers failing expected output schema.",
        "howToFix": "Wrap with ROUND(AVG(salary), 2).",
        "mistake": "Missing ROUND function",
        "whyItHappens": "Overlooking precision requirements in prompt specifications."
      }
    ]
  },
  "52": {
      ...
      "steps": [
        {
          ...
          "vars": {
            "Phase": "FROM students",
            "Action": "Read student score records"
          },
          "explanation": "Scans students table (Rahul: 10A/90, Priya: 10A/80, Amit: 10B/70, Neha: 10B/75, Rohan: 10C/95, Sneha: 10C/90)."
        },
        {
          "step": 2,
          "lineNumber": 4,
          "vars": {
            "Phase": "GROUP BY class_name",
            "Action": "Cluster rows by classroom"
          },
          "explanation": "Clusters students into classroom groups: 10A [90, 80], 10B [70, 75], 10C [95, 90]."
        },
        {
          "step": 3,
          "lineNumber": 2,
          "vars": {
            "Phase": "AVG(marks)",
            "Action": "Compute class averages"
          },
          "explanation": "Calculates average mark per class: 10A = 85.0, 10B = 72.5, 10C = 92.5."
        },
        {
          "step": 4,
          "lineNumber": 5,
          "vars": {
            "Phase": "HAVING AVG(marks) > 80",
            "Action": "Filter grouped metrics"
          },
          "explanation": "Keeps classes with average > 80: 10A (85.0) ✅, 10C (92.5) ✅. Filters out 10B (72.5) ❌."
        },
        {
          "step": 5,
          "lineNumber": 2,
          "vars": {
            "Phase": "ROUND(..., 2)",
            "Action": "Format decimal precision"
          },
          "explanation": "Rounds average marks to 2 decimal places (10C: 92.50, 10A: 85.00)."
        },
        {
          "step": 6,
          "lineNumber": 1,
          "vars": {
            "Phase": "SELECT class_name, average_marks",
            "Action": "Project candidate columns"
          },
          "explanation": "Prepares projection columns with alias average_marks."
        },
        {
          "step": 7,
          "lineNumber": 6,
          "vars": {
            "Phase": "ORDER BY average_marks DESC",
            "Action": "Sort descending"
          },
          "explanation": "Ranks classes descending: 1. 10C (92.50), 2. 10A (85.00)."
        }
      ]
    },
    "qas": [
      {
        "id": "q-52-1",
        "category": "💡 Interview Notes",
        "question": "Why is HAVING used instead of WHERE?",
        "whatInterviewerChecks": "Group-level filter vs row-level filter understanding.",
        "bestReplyScript": "WHERE filters raw rows before aggregation, so aggregate functions like AVG() cannot be evaluated in WHERE. HAVING filters collapsed groups after GROUP BY evaluation.",
        "commonMistakesToAvoid": "Writing WHERE AVG(marks) > 80.",
        "keyPoints": ["WHERE filters rows before GROUP BY", "HAVING filters groups after GROUP BY", "AVG() requires HAVING"],
        "codeSnippet": "HAVING AVG(marks) > 80"
      },
      {
        "id": "q-52-2",
        "category": "💡 Interview Notes",
        "question": "What happens if you write WHERE marks > 80 instead of HAVING AVG(marks) > 80?",
        "whatInterviewerChecks": "Pre-aggregation row filtering pitfalls.",
        "bestReplyScript": "WHERE marks > 80 discards individual students who scored <= 80 before calculating the average. This distorts the class average by ignoring low marks entirely rather than evaluating the true class mean.",
        "commonMistakesToAvoid": "Confusing student-level threshold filters with class-level average filters.",
        "keyPoints": ["WHERE marks > 80 filters individual students", "Distorts class average calculation", "HAVING tests the true cohort average"],
        "codeSnippet": "HAVING AVG(marks) > 80"
      },
      {
        "id": "q-52-3",
        "category": "💡 Interview Notes",
        "question": "Does AVG() count NULL marks in its denominator?",
        "whatInterviewerChecks": "NULL propagation semantics in SQL aggregate functions.",
        "bestReplyScript": "No. AVG() ignores NULL values completely in both sum numerator and count denominator. If a class has marks 90, 80, and NULL, AVG is (90+80)/2 = 85.",
        "commonMistakesToAvoid": "Assuming NULL marks are treated as 0 in AVG().",
        "keyPoints": ["AVG() excludes NULLs completely", "Denominator excludes NULL rows", "Use COALESCE(marks, 0) to treat NULL as 0"],
        "codeSnippet": "AVG(COALESCE(marks, 0))"
      },
      {
        "id": "q-52-4",
        "category": "💡 Interview Notes",
        "question": "What is the logical execution order of this query?",
        "whatInterviewerChecks": "SQL query execution lifecycle.",
        "bestReplyScript": "1. FROM students -> 2. GROUP BY class_name -> 3. Evaluate AVG(marks) -> 4. HAVING AVG(marks) > 80 -> 5. ROUND(..., 2) -> 6. SELECT class_name, average_marks -> 7. ORDER BY average_marks DESC.",
        "commonMistakesToAvoid": "Assuming HAVING runs after SELECT or ORDER BY.",
        "keyPoints": ["FROM -> GROUP BY -> AVG -> HAVING -> ROUND -> SELECT -> ORDER BY"],
        "codeSnippet": "FROM -> GROUP BY -> HAVING -> SELECT -> ORDER BY"
      },
      {
        "id": "q-52-5",
        "category": "💡 Interview Notes",
        "question": "How would you rewrite this query using a CTE?",
        "whatInterviewerChecks": "CTE vs HAVING refactoring flexibility.",
        "bestReplyScript": "WITH ClassAverages AS (SELECT class_name, ROUND(AVG(marks), 2) AS average_marks FROM students GROUP BY class_name) SELECT * FROM ClassAverages WHERE average_marks > 80 ORDER BY average_marks DESC;",
        "commonMistakesToAvoid": "Using HAVING inside the outer query when querying a CTE.",
        "keyPoints": ["CTEs materialize group averages first", "Outer query uses normal WHERE on CTE alias", "Equivalent execution plan"],
        "codeSnippet": "WITH ClassAverages AS (...)"
      }
    ],
    "questions": [
      {
    ...
    "mistakes": [
      {
        "id": "m-52-1",
        "title": "1. Using WHERE with AVG(marks)",
        "description": "Writing WHERE AVG(marks) > 80.",
        "badSnippet": "SELECT class_name, AVG(marks) FROM students WHERE AVG(marks) > 80 GROUP BY class_name;",
        "failingInput": "Query execution",
        "consequence": "Causes a SQL syntax error: aggregate functions are invalid in WHERE.",
        "howToFix": "Move aggregate filter to HAVING clause.",
        "mistake": "Using aggregate in WHERE",
        "whyItHappens": "Confusing row-level filtering with group-level filtering."
      },
      {
        "id": "m-52-2",
        "title": "2. Using WHERE marks > 80 Instead of HAVING",
        "description": "Filtering student marks before calculating class average.",
        "badSnippet": "SELECT class_name, ROUND(AVG(marks), 2) FROM students WHERE marks > 80 GROUP BY class_name;",
        "failingInput": "Class with scores [90, 30] (true average = 60)",
        "consequence": "Ignores score 30, evaluates average as 90, and incorrectly includes the class.",
        "howToFix": "Use HAVING AVG(marks) > 80.",
        "mistake": "Filtering raw rows instead of group metric",
        "whyItHappens": "Misunderstanding when WHERE vs HAVING filters occur."
      },
      {
        "id": "m-52-3",
        "title": "3. Forgetting GROUP BY class_name",
        "description": "Omitting GROUP BY clause entirely.",
        "badSnippet": "SELECT class_name, ROUND(AVG(marks), 2) FROM students HAVING AVG(marks) > 80;",
        "failingInput": "Multi-class school database",
        "consequence": "Collapses all students into a single overall school average row.",
        "howToFix": "Add GROUP BY class_name.",
        "mistake": "Missing GROUP BY clause",
        "whyItHappens": "Forgetting that class average requires grouping by class."
      },
      {
        "id": "m-52-4",
        "title": "4. Using COUNT() Instead of AVG()",
        "description": "Writing COUNT(marks) > 80.",
        "badSnippet": "SELECT class_name, COUNT(marks) FROM students GROUP BY class_name HAVING COUNT(marks) > 80;",
        "failingInput": "Class student counts vs mark values",
        "consequence": "Filters classes with >80 students instead of classes with average mark >80.",
        "howToFix": "Use AVG(marks) > 80.",
        "mistake": "Using wrong aggregate function",
        "whyItHappens": "Confusing record count threshold with score average threshold."
      },
      {
        "id": "m-52-5",
        "title": "5. Forgetting ORDER BY average_marks DESC",
        "description": "Omitting ORDER BY or sorting ASC.",
        "badSnippet": "SELECT class_name, ROUND(AVG(marks), 2) AS average_marks FROM students GROUP BY class_name HAVING AVG(marks) > 80;",
        "failingInput": "Sorted honor roll report requirement",
        "consequence": "Returns qualifying classes in arbitrary database order instead of top-first.",
        "howToFix": "Append ORDER BY average_marks DESC.",
        "mistake": "Missing or inverted ORDER BY clause",
        "whyItHappens": "Overlooking sorting specification in problem prompt."
      }
    ]
  },
  "53": {
      ...
      "steps": [
        {
          ...
          "vars": {
            "Phase": "FROM employees",
            "Action": "Read employee records"
          },
          "explanation": "Scans employees table (John: IT/Bangalore, Alice: IT/Bangalore, Bob: IT/Delhi, Emma: HR/Delhi, David: HR/Mumbai, Sophia: HR/Mumbai)."
        },
        {
          "step": 2,
          "lineNumber": 5,
          "vars": {
            "Phase": "GROUP BY department_name, city",
            "Action": "Cluster rows into composite pairs"
          },
          "explanation": "Clusters employees by unique (department, city) pairs: [HR, Delhi], [HR, Mumbai], [IT, Bangalore], [IT, Delhi]."
        },
        {
          "step": 3,
          "lineNumber": 3,
          "vars": {
            "Phase": "COUNT(*)",
            "Action": "Count headcount per pair"
          },
          "explanation": "Counts headcount per group: [HR, Delhi] = 1, [HR, Mumbai] = 2, [IT, Bangalore] = 2, [IT, Delhi] = 1."
        },
        {
          "step": 4,
          "lineNumber": 1,
          "vars": {
            "Phase": "SELECT department_name, city, employee_count",
            "Action": "Project candidate columns"
          },
          "explanation": "Projects department_name, city, and aliased employee_count."
        },
        {
          "step": 5,
          "lineNumber": 7,
          "vars": {
            "Phase": "ORDER BY department_name, city",
            "Action": "Multi-column sort"
          },
          "explanation": "Sorts by department_name alphabetically first (HR then IT), then resolves ties by city alphabetically (Delhi then Mumbai; Bangalore then Delhi)."
        }
      ]
    },
    "qas": [
      {
        "id": "q-53-1",
        "category": "💡 Interview Notes",
        "question": "How does GROUP BY department_name, city form groups?",
        "whatInterviewerChecks": "Understanding of multi-column aggregate key partitioning.",
        "bestReplyScript": "SQL evaluates the composite combination of both values for every row. A distinct group is formed for every unique (department_name, city) pair present in the dataset.",
        "commonMistakesToAvoid": "Thinking SQL creates separate un-nested result sets for department and city.",
        "keyPoints": ["Groups formed on unique (col1, col2) pairs", "Row count equals number of distinct pairs", "Essential for matrix breakdown reporting"],
        "codeSnippet": "GROUP BY department_name, city"
      },
      {
        "id": "q-53-2",
        "category": "💡 Interview Notes",
        "question": "What happens if a selected column is not included in GROUP BY?",
        "whatInterviewerChecks": "ANSI SQL strict mode rules for non-aggregated columns.",
        "bestReplyScript": "In standard ANSI SQL, selecting an unaggregated column that is not part of the GROUP BY clause throws a syntax error because the database engine cannot determine which row's value to display for that group.",
        "commonMistakesToAvoid": "Selecting columns like first_name without grouping or aggregating them.",
        "keyPoints": ["All selected non-aggregate columns MUST be in GROUP BY", "Throws invalid GROUP BY expression error", "Fix by adding to GROUP BY or wrapping in MAX/MIN"],
        "codeSnippet": "SELECT department_name, city"
      },
      {
        "id": "q-53-3",
        "category": "💡 Interview Notes",
        "question": "Does column order in GROUP BY change the output groups or counts?",
        "whatInterviewerChecks": "Set-theoretical behavior of GROUP BY keys.",
        "bestReplyScript": "No. GROUP BY department_name, city produces the exact same aggregate buckets and headcount as GROUP BY city, department_name. However, the SELECT clause determines column display order, and ORDER BY determines output row sorting.",
        "commonMistakesToAvoid": "Thinking changing GROUP BY column sequence changes calculated headcount.",
        "keyPoints": ["Aggregate buckets are identical regardless of key order", "SELECT clause dictates display order", "ORDER BY dictates sort order"],
        "codeSnippet": "GROUP BY department_name, city"
      },
      {
        "id": "q-53-4",
        "category": "💡 Interview Notes",
        "question": "How does ORDER BY department_name, city work?",
        "whatInterviewerChecks": "Multi-column sorting mechanics.",
        "bestReplyScript": "It sorts the result set primarily by department_name alphabetically. For rows sharing the same department_name, it breaks ties by sorting city alphabetically.",
        "commonMistakesToAvoid": "Thinking ORDER BY only sorts the second column.",
        "keyPoints": ["Primary sort on first column", "Secondary tie-breaker sort on second column", "Guarantees deterministic presentation"],
        "codeSnippet": "ORDER BY department_name, city"
      },
      {
        "id": "q-53-5",
        "category": "💡 Interview Notes",
        "question": "What index optimizes this multi-column GROUP BY query?",
        "whatInterviewerChecks": "Database indexing strategies for multi-column aggregation.",
        "bestReplyScript": "A composite index on (department_name, city) allows an Index-Only Scan. The database engine can perform stream aggregation directly over index leaves without fetching table heap pages.",
        "commonMistakesToAvoid": "Creating two separate single-column indexes.",
        "keyPoints": ["Composite index (department_name, city) is optimal", "Enables Loose/Stream Index Scan", "Eliminates expensive sorting or hashing"],
        "codeSnippet": "CREATE INDEX idx_dept_city ON employees(department_name, city)"
      }
    ],
    "questions": [
      {
    ...
    "mistakes": [
      {
        "id": "m-53-1",
        "title": "1. Selecting Non-Grouped Unaggregated Columns",
        "description": "Selecting an extra column like employee_name without including it in GROUP BY.",
        "badSnippet": "SELECT department_name, city, employee_name, COUNT(*) FROM employees GROUP BY department_name, city;",
        "failingInput": "ANSI SQL strict mode database execution",
        "consequence": "Causes SQL error: employee_name must appear in GROUP BY or be used in an aggregate function.",
        "howToFix": "Remove employee_name or add it to GROUP BY.",
        "mistake": "Unaggregated column in SELECT",
        "whyItHappens": "Assuming SQL will pick a random employee name for the group."
      },
      {
        "id": "m-53-2",
        "title": "2. Forgetting the Second Column in GROUP BY",
        "description": "Grouping only by department_name while selecting both department_name and city.",
        "badSnippet": "SELECT department_name, city, COUNT(*) FROM employees GROUP BY department_name;",
        "failingInput": "Multi-city department organization",
        "consequence": "Throws GROUP BY error or displays arbitrary city names.",
        "howToFix": "Include city in GROUP BY: GROUP BY department_name, city.",
        "mistake": "Incomplete GROUP BY clause",
        "whyItHappens": "Forgetting that every non-aggregated SELECT column must be grouped."
      },
      {
        "id": "m-53-3",
        "title": "3. Confusing WHERE with GROUP BY",
        "description": "Using WHERE department_name, city instead of GROUP BY.",
        "badSnippet": "SELECT department_name, city, COUNT(*) FROM employees WHERE department_name, city;",
        "failingInput": "Query execution",
        "consequence": "Causes syntax error; WHERE filters rows based on boolean conditions, it does not group data.",
        "howToFix": "Use GROUP BY department_name, city.",
        "mistake": "Confusing filtering with grouping",
        "whyItHappens": "Misunderstanding relational algebra operations."
      },
      {
        "id": "m-53-4",
        "title": "4. Forgetting ORDER BY Clause",
        "description": "Omitting ORDER BY department_name, city.",
        "badSnippet": "SELECT department_name, city, COUNT(*) AS employee_count FROM employees GROUP BY department_name, city;",
        "failingInput": "Sorted departmental city headcount report requirement",
        "consequence": "Outputs department-city rows in non-deterministic random database order.",
        "howToFix": "Append ORDER BY department_name, city.",
        "mistake": "Missing ORDER BY clause",
        "whyItHappens": "Assuming GROUP BY automatically sorts output rows."
      },
      {
        "id": "m-53-5",
        "title": "5. Forgetting COUNT(*)",
        "description": "Omitting the COUNT(*) function.",
        "badSnippet": "SELECT department_name, city FROM employees GROUP BY department_name, city;",
        "failingInput": "Headcount metric requirement",
        "consequence": "Returns unique department-city pairs (like DISTINCT) without calculating employee count.",
        "howToFix": "Add COUNT(*) AS employee_count to SELECT.",
        "mistake": "Missing aggregate function",
